    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            if FLAGS.train_or_restore == 'R':
                ...
                print("The format of your input is legal, now loading to next step...")
            ...
            for iii in range(FLAGS.epochs):
                ...
                print("epochs {0}: acc {1:g}  ".format((iii +1), acc))
                ...
                print("Epoch {0} has finished!".format(iii + 1))

            # logger.info("running time analysis: epoch{0}, avg_time{1}".format(len(run_time), np.mean(run_time)))
            print("running time analysis: epoch{0}, avg_time{1}".format(len(run_time), np.mean(run_time)))
            # logger.info("max: acc{0:g} ".format(m_acc))
            print("max: acc{0:g} ".format(m_acc))
